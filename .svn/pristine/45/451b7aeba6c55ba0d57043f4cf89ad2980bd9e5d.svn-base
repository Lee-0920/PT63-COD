...
static xSemaphoreHandle s_semTemp;
...
void EnvTempCollect_Init(void)
{
    ...
    vSemaphoreCreateBinary(s_semTemp);
}
/**
 * @briefSTM32 IIC接口初始化
 * @param
...
float EnvironmentTemp_Get(void)
{
    ...
    xSemaphoreTake(s_semTemp, portMAX_DELAY);
    ...
    if(TRUE == EnvTempCollect_I2CRequestReadReg()) // 1 请求
    {
        ...
        if(TRUE == EnvTempCollect_I2CReceive(2, buffer))    // 2 读取AD值
        {
            ...
            TRACE_CODE("\n Get eTemp Success : ");
            System_PrintfFloat(TRACE_LEVEL_CODE,  temperature, 2);
        }
    }
    if((-3000) == temperature)  // 说明IIC通信错误或者传感器件工作不正常
    {
        ...
        if(TRUE == EnvTempCollect_I2CRequestReadReg()) // 1 请求
        {
            ...
            {
                temperature =(float)((buffer[0]<<4 | buffer[1]>>4) & 0xfff) * TMP75_RESOLUTION;    // 3 转换
                TRACE_WARN("\n Get eTemp Retry Success: ");
                System_PrintfFloat(TRACE_LEVEL_WARNING,  temperature, 2);
            }
            else
            {
                TRACE_ERROR("\n Get eTemp Retry Fail: ");
                System_PrintfFloat(TRACE_LEVEL_ERROR,  temperature, 2);
            }
        }
        else
        {
            TRACE_ERROR("\n Get eTemp Retry Wrong : ");
            System_PrintfFloat(TRACE_LEVEL_ERROR,  temperature, 2);
        }
    }

    if(temperature > -100)  //更新正确的温度值
    {
        s_rightTemp = temperature;
    }
    else  //使用上次正确的值
    {
        temperature = s_rightTemp;
    }

    xSemaphoreGive(s_semTemp);
