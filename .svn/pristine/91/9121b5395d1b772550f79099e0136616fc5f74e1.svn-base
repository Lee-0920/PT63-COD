...
//#define METERLED_LED0_PIN       GPIO_Pin_2
...
{
    if(Hardware_v14 == HardwareType_GetValue())
    {
        g_meterLED[0].pin = GPIO_Pin_4;
        g_meterLED[0].port = GPIOA;
        g_meterLED[0].rcc = RCC_AHB1Periph_GPIOA;
    }
    else
    {
        g_meterLED[0].pin = GPIO_Pin_2;
        g_meterLED[0].port = GPIOA;
        g_meterLED[0].rcc = RCC_AHB1Periph_GPIOA;
    }

    g_meterLED[1].pin = GPIO_Pin_3;
...
void MeterLED_Init(void)
{
    ...
    GPIO_SetBits(g_meterLED[0].port, g_meterLED[0].pin);

    GPIO_InitStructure.GPIO_Pin = g_meterLED[1].pin;
    GPIO_Init(g_meterLED[1].port, &GPIO_InitStructure);
    GPIO_SetBits(g_meterLED[1].port, g_meterLED[1].pin);
}
