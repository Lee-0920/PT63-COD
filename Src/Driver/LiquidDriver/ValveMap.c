...
void ValveMap_Init(Valve *valve)
{
    ...
    s_valveDir[0].pin = GPIO_Pin_6;
    s_valveDir[0].port = GPIOC;
    s_valveDir[0].rcc = RCC_AHB1Periph_GPIOC;
    ValveDriver_Init(&s_valveDir[0]);
    ValveDriver_Control(&s_valveDir[0], VAlVE_OPEN);

    s_valveDir[1].pin = GPIO_Pin_15;
    s_valveDir[1].port = GPIOD;
    s_valveDir[1].rcc = RCC_AHB1Periph_GPIOD;
    ValveDriver_Init(&s_valveDir[1]);
    ValveDriver_Control(&s_valveDir[1], VAlVE_OPEN);
